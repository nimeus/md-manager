...
- **Status:** ✅ Phase 1 MVP · ✅ Phase 2 remote MCP + OAuth JWT · ✅ Phase 3 web app code (`frontend/`, Next.js BFF — **not built here**: npm unreachable) · ✅ Phase 4 semantic search (pgvector + env-driven embeddings, **OpenRouter default** — [docs/embeddings.md](docs/embeddings.md); needs a real key + `CREATE EXTENSION vector`). ✅ **Native Claude/ChatGPT connector** via a built-in OAuth 2.1 authorization server (`MDM_OAUTH_MODE=builtin`, migration 0009) — [docs/oauth-builtin.md](docs/oauth-builtin.md); Logto remains an optional external-JWT alternative ([docs/oauth-logto.md](docs/oauth-logto.md)).

## Stack
Rust cargo workspace · Postgres 17 (local via Homebrew; no Docker) · Next.js (Phase 3) · built-in OAuth 2.1 AS for native connectors (Logto optional, external).
...
apps/api       (mdm-api)    Axum HTTP API: REST (v1) + remote MCP at POST /mcp + built-in OAuth 2.1 AS (oauth_server.rs) + resource server (state.rs, mcp.rs) + external-JWT validation (oauth.rs). Bin: mdm-api
...
migrations/                 sqlx migrations (0001_init … 0009_oauth_server: built-in OAuth 2.1 AS — clients, codes, access/refresh tokens)
...
Native connector (built-in OAuth AS): `MDM_OAUTH_MODE=builtin`, `MDM_PUBLIC_URL`, `MDM_WEB_URL`, `MDM_OAUTH_{ACCESS,REFRESH,CODE,REQUEST}_TTL_SECS`, `MDM_OAUTH_DCR_PER_HOUR` — see [docs/oauth-builtin.md](docs/oauth-builtin.md).
CLI/MCP: `MDM_API_URL`, `MDM_API_KEY`, `MDM_BOOTSTRAP_TOKEN`. CLI also reads `~/.config/md-manager/config.json` (`mdm auth login`).

## Notes / known follow-ups
- MCP is a hand-rolled stdio JSON-RPC server (robust, spec-compliant); the remote MCP + built-in OAuth 2.1 authorization server are hand-rolled in `apps/api` (`oauth_server.rs` / `mcp.rs`). Could adopt `rmcp` later.
