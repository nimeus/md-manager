...
async fn member_roles_and_removal() {
    ...
    db.create_invitation(&ctx, "bob@x.com", OrgRole::Member).await.unwrap();
    let bob = db.provision_google_user("g_bob", "bob@x.com", "Bob").await.unwrap();

    let members = db.list_members(&ctx).await.unwrap();
    assert_eq!(members.len(), 2);
    assert!(members.iter().any(|m| m.user_id == bob.user_id && m.role == OrgRole::Member));

    // Promote Bob to admin.
    db.update_member_role(&ctx, bob.user_id, OrgRole::Admin).await.unwrap();
    assert!(
        db.list_members(&ctx)
            .await
            .unwrap()
    ...
    assert!(db.update_member_role(&ctx, alice.id, OrgRole::Member).await.is_err());
...
async fn accept_invitation_by_link_is_single_use() {
    ...
    let inv = db.create_invitation(&ctx, "carol@x.com", OrgRole::Viewer).await.unwrap();
    let carol = db
        .provision_google_user("g_carol", "carol-other@x.com", "Carol")
        .await
        .unwrap();

    let joined = db.accept_invitation_by_token(carol.user_id, &inv.token).await.unwrap();
    ...
    assert!(db.accept_invitation_by_token(carol.user_id, &inv.token).await.is_err());
    // Garbage token → not found.
    assert!(db.accept_invitation_by_token(carol.user_id, "inv_deadbeef").await.is_err());
}
